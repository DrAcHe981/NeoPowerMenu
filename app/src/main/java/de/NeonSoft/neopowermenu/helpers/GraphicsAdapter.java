						...
						mImageLoader.displayImage((mGraphics.get(position).getFile().startsWith("file://") ? "" : "file://") + mGraphics.get(position).getFile(),
								holder.imgQueue, new SimpleImageLoadingListener() {
										...
										public void onLoadingFailed(String imageUri, View view, FailReason failReason) {
												Log.e("NPM:graphicsList", "Failed to load image '" + imageUri + "': " + failReason.toString());
										}
								});
				} else if (mGraphics.get(position).getRessource() != -1) {
						holder.LoadingBar.setVisibility(View.GONE);
						holder.imgQueue.setVisibility(View.GONE);
						holder.imgQueue.setPadding((int) Math.max(PreferencesGraphicsFragment.float_padding * 4, helper.convertDpToPixel(20, mActivity)), (int) Math.max(PreferencesGraphicsFragment.float_padding * 4, helper.convertDpToPixel(20, mActivity)), (int) Math.max(PreferencesGraphicsFragment.float_padding * 4, helper.convertDpToPixel(20, mActivity)), (int) Math.max(PreferencesGraphicsFragment.float_padding * 4, helper.convertDpToPixel(20, mActivity)));
						if(mGraphics.get(position).getName().contains(".")) {
								try
								{
